...
export async function cli(args: string[]): Promise<void> {
  ...
  if (argv.verbose) {
    ...
    logger.debug(`Quality: ${argv.quality ?? 'default'}`)
    logger.debug(`Remove metadata: ${argv.removeMetadata ?? false}`)
    logger.debug(`Convert WebP: ${argv.convertWebp ?? false}`)
    logger.debug(`Convert AVIF: ${argv.convertAvif ?? false}`)
    logger.debug(`Workers: ${argv.workers ?? 1}`)
  }

  logger.info('✅ CLI is working! (Phase 1* complete - TypeScript)')
}
